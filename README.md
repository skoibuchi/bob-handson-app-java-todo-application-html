...
> **バージョンについて**
> Bob 1.0 と Bob 2.0 ではモード構成が異なります。該当するバージョンのセクションを参照してください。
...
- Bob 1.0 の CodeモードとAdvancedモードに相当
...
本リポジトリの `.bob/rules-plan/rule.md` に設定済みのため、追加作業は不要です。
...
2. **Bob 1.0: Codeモードでない場合は、BobがCodeモードへの変更を要求してくるので、承認します**  
   **Bob 2.0: Agentモードでない場合は、BobがAgentモードへの変更を要求してくるので、承認します**
...
1. **Bob 1.0: Advancedモードに設定します**  
   **Bob 2.0: Agentモードに設定します**
...
4. **Bobにテストコード実行を実施させ、最後にテストレポートを出力させます**
   ...
   ※ テストはブラウザを使用するため、Bob 1.0 はAdvancedモード、Bob 2.0 はAgentモードで実行されることを確認してください。
