...
実装完了後、Bobに実装したコードがコーディング規約に沿っているか採点させます。

### 手順

1. **`3. 実装`の続きのチャットで、Bobに採点をリクエストします**
   ```
   実装したコードが .bob/rules-agent/coding-rules.md のコーディング規約に沿っているか、100点満点で採点してください。
   ```

2. **Bobが規約への準拠度を項目ごとに評価し、点数とコメントを出力します**
   - 各ルール（コメント規則・変数命名規則・エラーメッセージ規則）ごとの評価が出力されます
   - 減点箇所がある場合は、具体的なファイルと行番号が示されます

3. **採点結果を確認し、規約違反があればBobに修正をリクエストします**
   ```
   指摘箇所を修正してください。
   ```

4. **修正後、再度採点させて点数が上がっていることを確認します**
...
2. 新機能の要件定義と設計
3. 設計書に基づいた実装
3.5. コーディング規約への準拠チェックと修正
4. 単体テストの計画と実施

Bobの各モードを適切に使い分けることで、効率的な開発が可能になります。

## トラブルシューティング（2026/06/12更新）
...
今後の開発において、 **Agent AIをどのようなシーンで活用できるか？** をテーマにグループディスカッションを行います。
下記のプロンプトのサンプルの【ディスカッション結果】をグループでのディスカッション内容に書き換えることで、Bobに発表資料を作らせる事も可能です。

```
【背景】
私たちはIT開発・保守チームで、コーディングエージェントのハンズオンを実施しました。
このスライドは、グループディスカッションで出た活用アイデアを発表するためのものです。
【ディスカッション結果】
グループ名: [グループ名を記載]
活用分野:
1. [活用分野1]
...
5. まとめ
【デザイン要件】
- テーマ: ビジネスプレゼンテーション向けのプロフェッショナルなデザイン
- カラースキーム: 青系を基調に
- フォント: 見やすいゴシック体
- 各スライドに適切な絵文字やアイコンを使用
- 箇条書きは3-5項目に絞り、簡潔に
- 図表やビジュアル要素を適宜追加
- 発表時間の3分に収まる枚数になるよう、適宜スライド構成を変更
【出力形式】
