...
### 1. 設計書の影響調査
- 追加要件が既存設計書のどの箇所に影響するかをBobに調査させる
- Askモードを使用し、ファイルを変更せずに影響箇所の把握のみ行う

### 2. 設計書の更新
- 影響調査の結果を基に、既存設計書の影響箇所を修正・追記する
- Planモードを使用し、設計書が適切に更新されていることを確認する

### 3. 実装
- Bobと実装
- 更新済み設計書に沿ったコードが生成されることを確認
...
Askモードにて、追加要件が既存設計書のどの箇所に影響するかをBobに調査させます。
Askモードではファイルの変更は行わず、影響箇所の把握のみを行います。

### 手順

1. **Askモードに設定**

2. **Bobに追加要件を伝え、既存設計書への影響を調査するようにリクエストを出します**
   ```
   期限が3日以内のタスクを通知する機能を追加したい。既存の設計書を確認し、どの箇所に影響があるか調査してください。
   ```

3. **Bobがプロジェクト内にあるファイルから情報を読み取ろうとするので、適宜承認していきます**

4. **Bobが影響箇所の調査結果をチャット上でレポートとして返答します**
   ファイルは一切変更されないことを確認してください。

5. **調査結果を確認し、影響箇所と修正内容を把握します**
   不明点があればBobに追加質問して内容を理解してから次のステップへ進みます。

---

## 2. 設計書の更新

影響調査の結果を基に、Planモードにて既存設計書の影響箇所を修正・追記します。

### 手順

1. **Planモードに設定**

2. **Bobに設計書の更新をリクエストします**
   ```
   影響調査の結果を基に、設計書の影響箇所を更新してください。
   ```
...
6. **適宜設計書が更新されるので、内容が問題なければ承認、問題があれば適宜Bobに伝えて修正を促します**
   設計書の更新が行われずに実装を開始しそうな時は、適宜Bobに設計書を更新するように指示してください。
...
1. 追加要件に対する既存設計書への影響調査（Askモード）
2. 設計書の更新 — 影響箇所の修正・追記（Planモード）
3. 更新済み設計書に基づいた実装（Agentモード）
4. 単体テストの計画と実施（PlanモードおよびAgentモード）
